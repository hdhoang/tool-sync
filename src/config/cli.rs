...
pub enum Command {
    ...
    ///
    /// You will need to set up completions manually, follow the instructions
    /// below. The exact config file locations might vary based on your
    /// system. Make sure to restart your shell before testing whether
    ...
    /// ### zsh
    ///
    /// Generate a `_tool` completion script and put it somewhere in your
    /// `$fpath`:
    ///
    ...
    ///
    /// `mkdir -Path (Split-Path -Parent $profile) -ErrorAction
    /// SilentlyContinue` `notepad $profile`
    ///
    ...
    Completion {
        ...
        /// If you want to rename the binary this option should be filled with
        /// the name you choose to rename `tool` to.
